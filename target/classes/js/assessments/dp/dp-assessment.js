...
// Prob 2
//      You are given a string s. Your task is to count the number of ways of
//      splitting s into three non-empty parts a, b and c (s = a + b + c) in
//      such a way that a + b, b + c and c + a are all different strings.
//
...
//      by their mean values, so that arrays with equal mean values are in the same
...
//      corresponding arrays (a[i], a[j], etc) all have the same mean. Return the
//      set of groups as an array of arrays, where the indices within each group
//      are sorted in ascending order, and the groups are sorted in ascending order
...
//      of every possible a[i] ∘ a[j], where a[i] ∘ a[j] is the concatenation of
//      the string representations of a[i] and a[j] respectively.
//
//      Example
